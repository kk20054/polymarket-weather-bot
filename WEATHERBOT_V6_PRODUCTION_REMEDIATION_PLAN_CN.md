...
## 2026-06-29 v6.7 城市上下文固定栏

本轮在 v6.6 的中间主板独立滚动基础上，继续补齐交易看板的浏览上下文：

- 中间城市页顶部的 `城市 / 站点 / 日期 / 最新温度 / 信号数 / 数据年龄` 改为 sticky，上下滚动查看趋势、分布、日志和数据明细时不会丢失当前城市和日期。
- 固定栏只放短状态，不放解释性段落；长说明继续使用 tab、details 和行展开承载。
- 日期同步使用当前 URL/城市页状态，避免用户滚动到底部时无法确认正在看的目标日。

验收标准：

- 中间主板滚动到下方数据明细时，顶部仍能看到城市、站点、日期和信号状态。
- sticky 固定栏不遮挡内容、不引入横向溢出。
- 该改动不影响左侧城市索引、右侧执行栏和后端刷新策略。

## 2026-06-29 v6.8 抓取日志时间线

本轮继续学习 PolyWX 的“模块下钻有真实排查信息”方式，把城市页日志从普通表格升级为数据链路时间线：

- `抓取日志` tab 改为时间线卡片，按天气、观测、盘口、信号、刷新等阶段归类。
- 每条日志保留时间、类型、消息和折叠数据 payload，便于排查 forecast、METAR/truth、CLOB orderbook、signal migration 哪一环失败。
- 错误/失败/timeout 等日志使用红色状态，天气/观测/盘口/信号使用不同状态色，提升扫描效率。
- 默认不展开 JSON payload，避免首屏变乱；需要排障时再展开。

验收标准：

- 抓取日志不再只是三列表格，而是能一眼看出阶段、时间和异常。
- 每条日志可展开查看底层数据，但默认保持收起。
- 移动端和桌面端均无横向溢出。

## 2026-06-29 v6.9 市场信号卡片

本轮把城市页 `市场信号` 从普通表格升级为可执行信号卡片，服务人工确认、模拟盘复盘和未来小额实盘审核：

- 每条信号默认展示 `BUY YES/观察`、目标温度桶、日期、盘口价格、spread/bid、模型概率、市场概率、edge、模型均值/离散度和 Polymarket 链接。
- 信号明细默认折叠，展开后展示建议金额、份额、策略分、truth/风险状态、fit 样本、near-lock、阻塞原因、提醒、质量标签、备注、market id 和 YES token。
- tab 顶部显示可执行、选中日期、模拟持仓、总信号计数，避免用户在表格里找状态。
- 选中日期信号会有独立标识，避免跨日期信号误读。

验收标准：

- 进入城市页 `市场信号` tab 后，不打开明细也能判断每条信号是否可以买、买哪个桶、价格和 edge 是多少。
- 展开单条信号能看到风控阻塞和 token/market 信息，支持手动下单核对。
- 该改动不改变算法、模拟成交或实盘执行逻辑。

## 2026-06-29 v6.10 主板滚动与偏差校准面板

本轮根据实机反馈继续修正 PolyWX 式城市页的可用性问题：

- 中间城市主板改为“顶部上下文固定 + 内容区独立滚动”，桌面端查看下方日志、信号、数据明细时不再被三栏 grid 锁住。
- `偏差统计` 从静态指标卡升级为校准面板：展示 MAE、bias、配对样本、truth 覆盖、最近误差条和选中日期校准状态。
- 最近误差以“实际最高 - 保存预测”为主读图方式，颜色只表达误差大小，避免把历史配对样本误读成盈利证明。
- Truth/provider、METAR、数据状态和用途说明默认折叠到“更多明细”，继续遵守“重点突出、解释隐藏”的 UI 原则。
- 该改动只提升看板诊断能力，不改变策略、模拟成交、实盘开关或后台刷新策略。

验收标准：

- 左侧、右侧和中间主板都能独立下拉，城市页底部信息可访问。
- 偏差统计 tab 能直接判断当前城市预测是否系统性偏高/偏低。
- 无配对样本时显示明确原因，而不是空白指标。
- 桌面和移动宽度均无横向溢出。

## 2026-06-29 v6.11 数据源模块最近记录

本轮继续强化城市页每个模块的下钻能力，让 `预报 / METAR / 历史观测 / 盘口信号` 不只是状态灯，而是能直接排障的数据卡：

- 每个数据源模块的 `明细` 展开项新增 `最近记录` 列表。
- 预报模块展示最近目标日、更新时间、best、ECMWF、HRRR、METAR 和来源。
- METAR 模块展示最近观测、best、湿度和来源。
- 历史观测模块展示实际最高温、provider、truth 层级和站点。
- 盘口/信号模块展示目标桶、BUY/观察状态、价格、edge 和第一条原因。
- 默认仍只展示简洁状态；展开后才显示排障信息，避免主界面重新变乱。

验收标准：

- 展开四个数据源模块，能看到最近记录而不是只有单行摘要。
- 最近记录不改变信号算法、模拟成交或实盘执行。
- 桌面和移动宽度均无横向溢出。
...
- 左侧城市栏不得在 `0/0` 信号时显示绿色“推荐关注”；只有存在可执行信号时才高亮推荐，否则降级为“等待抓取/当前城市”，避免交易语义误导。
- 左侧城市索引需要承担站点证据导航职责：每个城市行直接显示预报快照、METAR、历史观测和湿度可用性，系统闸门和长解释继续收进折叠区。
- 中间城市页头部必须先显示站点、目标日期、预测、METAR、历史证据覆盖和信号状态，帮助操作者在进入图表前先完成“这页有没有可用证据”的判断。
- 城市页桌面端只保留一个主标题；WeatherPanel 内部不得重复城市名和站点标签，内部顶部只负责日期切换、外部链接和补历史等局部操作。
- 图表标题栏只放证据状态和必要控件；读图说明、口径说明和教学文字统一下沉到折叠明细，避免遮挡核心天气趋势。
