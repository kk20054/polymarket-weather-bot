...
- 10 个独立预测与结算配对样本后开始渐进偏差修正；模拟发现不再因为尚未达到 20 个样本而停用；
...
2. 10 个样本起：动态权重开始吸收近期 MAE；加性偏差按 `n / (n + 10)` 向零先验收缩，并限制在 `±2.5°C`。
3. 20 个及以上样本：属于成熟校准证据，但仍不代表可实盘。
...
当前偏差表按城市/站点与模型家族训练，尚未进一步拆成 D+0、D+1、D+2 提前量桶。按提前量分层、指数时间衰减和天气形势特征是后续校准升级项，必须通过 walk-forward 回放后再替换当前版本。
...
2. 输入模拟本金和单笔上限。
3. 单选一个入场策略并选择退出方式。
4. 设置“最低交易优势”；默认 `8%`，保存后同时约束信号生成和成交前复核。
4. 确认顶部调度器正在运行。
5. 点击“启动策略”。
