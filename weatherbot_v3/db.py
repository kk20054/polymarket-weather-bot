...
def list_signal_decisions(
    ...
    *,
    ...
    for row in rows:
        ...
        raw_decision = _loads_obj(row.get("raw_json"))
        row["paper_gate_reasons"] = _loads_list(raw_decision.get("paper_gate_reasons")) or list(row["gate_reasons"])
        row["live_gate_reasons"] = _loads_list(raw_decision.get("live_gate_reasons"))
