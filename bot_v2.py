...
def get_weather_event_url(city_slug, date_str):
    try:
        dt = datetime.strptime(date_str, "%Y-%m-%d")
        slug = f"highest-temperature-in-{city_slug}-on-{MONTHS[dt.month - 1]}-{dt.day}-{dt.year}"
        return f"https://polymarket.com/event/{slug}"
    except Exception:
        return "https://polymarket.com/markets"
...
def print_manual_order_instructions(signal, loc, date, horizon, bucket_label, event_url):
    """Print a human-confirmed order ticket. This does not place real orders."""
    forecast_src = (signal.get("forecast_src") or "unknown").upper()
    yes_token_id = signal.get("yes_token_id") or "unknown"
    print("         manual order:")
    print(f"           URL:      {event_url}")
    print(f"           Market:   {signal['question']}")
    print("           Action:   BUY YES")
    print(f"           Limit:    ${signal['entry_price']:.3f} or lower")
    print(f"           Amount:   ${signal['cost']:.2f} max")
    print(f"           Shares:   about {signal['shares']:.2f} YES shares")
    print(f"           Bid/Ask:  ${signal['bid_at_entry']:.3f} / ${signal['entry_price']:.3f} "
            ...
            if not mkt.get("position") and forecast_temp is not None and hours >= MIN_HOURS:
                ...
                if best_signal:
                    ...
                    if not skip_position and best_signal["entry_price"] < MAX_PRICE:
                        ...
                        if log_signal:
                            try:
                                best_signal["city"] = city_slug
                                log_signal(best_signal, loc, date, horizon, bucket_label, event_url)
                            except Exception as e:
                                print(f"  [WARN] Could not save dashboard signal: {e}")
...
def print_status():
    ...
    if open_pos:
        ...
        for m in open_pos:
            ...
            if MANUAL_ORDER_INSTRUCTIONS:
                event_url = pos.get("event_url") or m.get("event_url") or get_weather_event_url(m["city"], m["date"])
                yes_token_id = pos.get("yes_token_id") or get_yes_token_id(pos["market_id"]) or "unknown"
                if event_url:
                    print(f"      URL: {event_url}")
                print(f"      Manual: BUY YES up to ${pos['cost']:.2f} @ ${pos['entry_price']:.3f} or lower"
                      f" | shares about {pos['shares']:.2f}"
                      f" | token {yes_token_id}")
