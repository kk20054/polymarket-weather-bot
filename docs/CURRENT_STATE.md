...
- 2026-07-11 / Truth audit: `truth_delta_audit` now persists `delta_hko_minus_iem`, maps Hong Kong HKO settlement truth to VHHH observations, and carries the city key. CLI now forwards WU `--force-rebuild`. Commit `cc90a90`.
...
- Verification: 183 `tests.test_v3_core` tests passed after the truth audit change; `git diff --check` passed.
