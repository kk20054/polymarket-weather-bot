...
class PolyWXDashboardContractTests(unittest.TestCase):
    ...
    def test_city_workbench_exposes_single_navigation_and_tabs(self):
        ...
        self.assertIn("groupedCityOptions.map", app)
        ...
        self.assertIn('data-testid="city-browse-mode"', app)
        self.assertIn('data-testid="city-groups"', app)
        self.assertIn("groupedCityOptions.map", app)
        self.assertIn("cityBrowseGroupLabel", app)
        self.assertIn("cityBrowseGroupRank", app)
    ...
    def test_layer7_visual_alignment_matches_polywx_priority(self):
        ...
        self.assertIn("策略候选", app)
        self.assertIn("hasStrategyRecommendations", app)
    ...
    def test_scheduler_status_contract_is_visible_to_dashboard(self):
        ...
        self.assertIn("启动策略", read_text("frontend/src/components/ExecutionWorkbench.tsx"))
    ...
    def test_tables_and_fetch_log_match_polywx_information_architecture(self):
        ...
        self.assertIn("时间 / 来源 / 状态 / 耗时 / 信息", panel)
        self.assertIn("fetchLog?: FetchLogRow[]", panel)
        self.assertIn("NormalizedFetchLogRow", panel)
        self.assertIn("showRunningProductionRefresh", panel)
        self.assertIn("[row.stage, row.source, row.message]", panel)
        self.assertIn("sourceLabel", panel)
        self.assertIn("暂无该城市的抓取记录", panel)
        self.assertIn("CloudCoverMeter", panel)
        self.assertIn("PrecipitationMetric", panel)
        self.assertIn("WindMetric", panel)
        self.assertIn('className="weather-panel ', panel)
        self.assertIn("weather-cloud-meter", css)
        self.assertIn(".weather-panel thead.sticky", css)
