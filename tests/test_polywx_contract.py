...
class PolyWXDashboardContractTests(unittest.TestCase):
    ...
    def test_layer7_visual_alignment_matches_polywx_priority(self):
        ...
        self.assertIn("–", panel)
        self.assertIn("WeatherBot 附加：盘口 / token / gate", panel)
        self.assertIn("DEB metadata", panel)
    ...
    def test_round5_i18n_city_status_delta_and_alpha_contract(self):
        ...
        self.assertIn("DeltaAuditPanel", app)
        self.assertIn("fetchTruthDeltaAudit", app)
        self.assertIn("fetchModelRepriceEvents", app)
        self.assertIn("activeMainView", app)
    ...
    def test_city_evidence_contract_is_exposed_for_polywx_generation(self):
        ...
        self.assertIn("evidenceSummary={selectedDateEvidence?.modules?.probability_buckets?.probability_summary}", panel)
        ...
        self.assertIn("marketSummary={layerMarketSummary ?? selectedDateEvidence?.modules?.market_buckets?.market_summary}", panel)
        self.assertIn("evidenceSummary?: CityEvidenceDiffStatsSummary", panel)
        self.assertIn("evidenceSummary?: CityEvidenceProbabilitySummary", panel)
        self.assertIn("marketSummary?: CityEvidenceMarketBucketSummary", panel)
